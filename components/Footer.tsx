...
const Footer = (): JSX.Element => {
	const mappedContacts: Array<JSX.Element> = contacts.map((contact: IContactItem) => (
	...
	const mappedCalendar: Array<JSX.Element> = calendars.map((calendar: ICalendarItem) => (
		<div className="contacts__calendar-col" key={ uuidv4() }>
			...
			<div className={(calendar.opened ? 'contacts__calendar-row green bold' : 'contacts__calendar-row red') + " f-row-center-center border pd-top-5 pd-bottom-5"} >
				{calendar.opened && (
		...
		<section id="contacts" className="contacts bg-white">
			<div className="contacts__container ctn f-row-between-start">
				...
				<div className="contacts__box w-30 pd-bottom-40">
					...
					<div
						className="contacts__calendar f-row-start-start w-100 border mg-top-20 o-hidden br-5"
					>
						{ mappedCalendar }
					</div>
				</div>
